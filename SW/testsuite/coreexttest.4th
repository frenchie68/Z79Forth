...
\ -------------------------------------------------------------
\ 0.14 21 July 2022 Updated first line of BUFFER: test as
\      recommended in issue 32
\ 0.13 28 October 2015
\      Replace <FALSE> and <TRUE> with FALSE and TRUE to avoid
\      dependence on Core tests
\      Moved SAVE-INPUT and RESTORE-INPUT tests in a file to
\      filetest.fth
\      Use of 2VARIABLE (from optional wordset) replaced with
\      CREATE.
\      Minor lower to upper case conversions.
\      Calls to COMPARE replaced by S= (in utilities.fth) to
\      avoid use of a word from an optional word set.
\      UNUSED tests revised as UNUSED UNUSED = may return FALSE
\      when an implementation has the data stack sharing unused
\      dataspace.
...
\      [COMPILE] tests removed because it is obsolescent in
\      Forth 2012
\ 0.10 1 August 2014
...
\     CONVERT test deleted - obsolete word removed from Forth
\     200X
...
\     .( .R 0<> 0> 2>R 2R> 2R@ :NONAME <> ?DO AGAIN C" CASE
\     COMPILE, ENDCASE ENDOF ERASE FALSE HEX MARKER NIP OF PAD
\     PARSE PICK REFILL RESTORE-INPUT ROLL SAVE-INPUT SOURCE-ID
\     TO TRUE TUCK U.R U> UNUSED VALUE WITHIN [COMPILE]

\ Words not tested or partially tested:
\     \ because it has been extensively used already and is,
\     hence, unnecessary
\     REFILL and SOURCE-ID from the user input device which are
\     not possible
\     when testing from a file such as this one
\     UNUSED (partially tested) as the value returned is system
\     dependent
\     Obsolescent words #TIB CONVERT EXPECT QUERY SPAN TIB as
\     they have been removed from the Forth 2012 standard

\ Results from words that output to the user output device have
\ to visually ecked for correctness. These are .R U.R .(
...
T{ :NONAME ( n -- 0,1,..n ) DUP IF DUP >R 1- RECURSE
     R> THEN ;
   CONSTANT RN1 -> }T
T{ 0 RN1 EXECUTE -> 0 }T
...
\ ------------------------------------------------------------
\ Cannot automatically test SAVE-INPUT and RESTORE-INPUT from a
\ console source
...
\ corrupt PAD. Minimum size of buffers for these are 33 chars
\ and (2*n)+2 chars respectively where n is number of bits per
\ cell
...
\ Z79Forth/A note: in block context, the first test invoking
\ PA1 will cause the rest of the buffer to be skipped and the
\ whole test suite to land abruptly. We can't have that!
...
\ REFILL and SOURCE-ID from the user input device can't be
\ tested from a file, can only be tested from a string via
\ EVALUATE
...
\ T{ SSQ3 DROP  8 CHARS + C@ ->  13 }T \ \r   CR  Carriage Ret.
\ T{ SSQ3 DROP  9 CHARS + C@ ->   9 }T \ \t   TAB Horiz. Tab
\ T{ SSQ3 DROP 10 CHARS + C@ ->  11 }T \ \v   VT  Vertical Tab
\ T{ SSQ3 DROP 11 CHARS + C@ ->  15 }T \ \x0F     Given Char
\ T{ SSQ3 DROP 12 CHARS + C@ ->  48 }T \ 0    0   Digit follows
\ T{ SSQ3 DROP 13 CHARS + C@ ->  31 }T \ \x1F     Given Char
\ T{ SSQ3 DROP 14 CHARS + C@ ->  97 }T \ a    a   Hex follow on
\ T{ SSQ3 DROP 15 CHARS + C@ -> 171 }T \ \xaB     Ins. Giv. Chr
\ T{ SSQ3 DROP 16 CHARS + C@ -> 120 }T \ x    x   NonhexFollows
...
\ T{ : SSQ7  S\" 111 : SSQ8 S\\\" 222\" EVALUATE ;
\    SSQ8 333" EVALUATE ; -> }T
\ T{ SSQ7 -> 111 222 333 }T
\ T{ : SSQ9  S\" 11 : SSQ10 S\\\" \\x32\\x32\" EVALUATE ;
\     SSQ10 33" EVALUATE ; -> }T
\ T{ SSQ9 -> 11 22 33 }T
