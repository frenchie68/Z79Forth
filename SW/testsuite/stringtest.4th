...
\ -------------------------------------------------------------
\ Version 0.13 13 Nov 2015 Interpretive use of S" replaced by
\             $" from utilities.fth
\         0.11 25 April 2015 Tests for REPLACES SUBSTITUTE
\             UNESCAPE added
\         0.6 1 April 2012 Tests placed in the public domain.
\         0.5 29 April 2010 Added tests for SEARCH and COMPARE
\             with all strings zero length (suggested by
\             Krishna Myneni).
\             SLITERAL test amended in line with
\             comp.lang.forth discussion
\         0.4 30 November 2009 <TRUE> and <FALSE> replaced with
\             TRUE and FALSE
\         0.3 6 March 2009 { and } replaced with T{ and }T
\         0.2 20 April 2007 ANS Forth words changed to upper
\             case
\         0.1 Oct 2006 First version released

\ -------------------------------------------------------------
\ The tests are based on John Hayes test program for the core
\ word set and requires those files to have been loaded

\ Words tested in this file are:
\     -TRAILING /STRING BLANK CMOVE CMOVE> COMPARE SEARCH
\     SLITERAL REPLACES SUBSTITUTE UNESCAPE
\
\ -------------------------------------------------------------
\ Assumptions, dependencies and notes:
\     - tester.fr (or ttester.fs), errorreport.fth and
\       utilities.fth have been included prior to this file
\     - the Core word set is available and tested
\     - COMPARE is case sensitive
\ -------------------------------------------------------------

CR .( *** Start of String word tests) CR

\ begin Z79Forth/A glue code.

: $" POSTPONE S" ; IMMEDIATE

\ end Z79Forth/A glue code.
...
\ -------------------------------------------------------------
TESTING SLITERAL (skipped)%

\ T{ HERE DUP S1 DUP ALLOT ROT SWAP CMOVE S1 SWAP DROP
\    2CONSTANT S1A -> }T
...
\ -------------------------------------------------------------
TESTING UNESCAPE (skipped)%

\ CREATE SUBBUF 48 CHARS ALLOT

\ \ $CHECK AND $CHECKN return f = 0 if caddr1 = SUBBUF and
\ \ string1 = string2
\ : $CHECK   ( caddr1 u1 caddr2 u2 -- f )  2SWAP OVER SUBBUF <>
\   >R COMPARE R> or ;
\ : $CHECKN  ( caddr1 u1 n caddr2 u2 -- f n )  ROT >R $CHECK
\   R> ;
...
\ T{ $" defg%mac1%hi" SUBST $" defgwxyzhi" $CHECKN
\    -> FALSE 1 }T
\ T{ $" 12" MAC2 REPLACES -> }T
\ T{ $" %mac1%mac2" SUBBUF 20 SUBSTITUTE $" wxyzmac2" $CHECKN
\    -> FALSE 1 }T
\ T{ $" abc %mac2% def%mac1%gh" SUBBUF 20 SUBSTITUTE
\    $" abc 12 defwxyzgh" $CHECKN -> FALSE 2 }T
\ T{ : REPL  ( caddr1 u1 "name" -- )  PARSE-NAME REPLACES ;
\    -> }T
...
\ T{ $" abc%%mac1%%%mac2%" SUBBUF 20 SUBSTITUTE $" abc%mac1%12"
\    $CHECKN -> FALSE 1 }T  \ Check substitution is single pass
\ T{ $" %mac3%" MAC3 REPLACES -> }T
\ T{ $" a%mac3%b" SUBBUF 20 SUBSTITUTE $" a%mac3%b" $CHECKN
\       -> FALSE 1 }T    \ Check non-recursive
\ T{ $" %%" MAC3 REPLACES -> }T
\ T{ $" abc%mac1%de%mac3%g%mac2%%%%mac1%hij" SUBBUF 30
\    SUBSTITUTE $" abcwxyzde%%g12%wxyzhij" $CHECKN
\    -> FALSE 4 }T
...
\ Check for errors
\ T{ $" abcd" SUBBUF 4 SUBSTITUTE $" abcd" $CHECKN
\    -> FALSE 0 }T  \ Just fits
\ T{ $" abcd" SUBBUF 3 SUBSTITUTE ROT ROT 2DROP 0<
\    -> TRUE }T     \ Just too long
...
\ Conditional test for overlapping strings, including the case
\ where caddr1 = caddr2. If a system cannot handle overlapping
\ strings it should return n < 0 with (caddr2 u2) undefined. If
\ it can handle them correctly it should return the usual
\ results for success. The following definitions apply the
\ appropriate tests depending on whether n < 0 or not.
...
\ Note that variables are used to avoid complicated stack
\ manipulations

\ VARIABLE sdest       \ Holds dest address for SUBSTITUTE
\ 20 constant ssize
\ 2VARIABLE $sresult
\ VARIABLE #subst      \ Hold output from SUBSTITUTE

\ sinit set ups addresses and inputs for SUBSTITUTE and saves
\ the output destination for check-subst srcn and destn are
\ offsets into the substitution buffer subbuf (caddr1 u1) is
\ the input string for SUBSTITUTE
...
\     n is the expected n if SUBSTITUTE succeeded with
\     overlapping buffers.

\ : check-subst  ( caddr1 u1 n -- f )
\    #subst @ 0<
\    IF DROP 2DROP TRUE EXIT THEN  \ SUBSTITUTE failed, test OK
\    #subst @ = >R
...
\ T{ $" abcde" 2 6 sinit
\    -> subbuf 2 chars + 5 subbuf 6 chars + ssize }T
\ T{ $" abcde" subbuf 2 chars + over compare -> 0 }T
\ T{ sdest @ -> subbuf 6 chars + }T

\ T{ -78 #subst ! 0 0 0 check-subst -> TRUE }T
\ T{ 5 #subst ! $" def" over sdest ! 2dup $sresult 2!
\    5 check-subst -> TRUE }T
\ T{ 5 #subst ! $" def" over sdest ! 2dup $sresult 2!
\    4 check-subst -> FALSE }T
\ T{ 5 #subst ! $" def" over sdest ! 2dup 1+ $sresult 2!
\    5 check-subst -> FALSE }T
\ T{ 5 #subst ! $" def" over sdest ! 2dup 1- $sresult 2!
\    3 check-subst -> FALSE }T
...
\ T{ $" a%mac3%b" 0 9 do-subst $" azyxwvutb" 1 check-subst
\    -> TRUE }T
\ T{ $" c%mac3%d" 0 3 do-subst $" czyxwvutd" 1 check-subst
\    -> TRUE }T
\ T{ $" e%mac2%f" 0 3 do-subst $" ezyxf"     1 check-subst
\    -> TRUE }T
\ T{ $" abcdefgh" 0 0 do-subst $" abcdefgh"  0 check-subst
\    -> TRUE }T
\ T{ $" i%mac3%j" 3 0 do-subst $" izyxwvutj" 1 check-subst
\    -> TRUE }T
\ T{ $" k%mac3%l" 9 0 do-subst $" kzyxwvutl" 1 check-subst
\    -> TRUE }T

\ Simulating a failing overlapping SUBSTITUTE

\ T{ $" pqrst" 2dup 0 0 do-subst -78 #subst ! 0 check-subst
\    -> TRUE }T

\ Using SUBSTITUTE to define a name whose (caddr u) is on
\ the stack
\ : $CREATE  ( caddr u -- )
\    S" name" REPLACES          ( -- )
