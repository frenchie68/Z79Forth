...
\ -------------------------------------------------------------
\ Version 0.13 13 Nov 2015 C6 rewritten to avoid use of CASE
\ etc and hence dependence on the Core extension word set.
...
\   undefined word used is $$qweqweqwert$$, if this happens to
\   be a valid word in your system change the definition of t7
\   below
...
: C5 1 2 3 4 ['] T5 CATCH     \ Test depth restored correctly
   DEPTH >R DROP 2DROP 2DROP
   R> ;                       \ after stack has been emptied
T{ C5 -> 5 }T
...
\ The 77 in T10 is necessary for the second ABORT" test as the
\ data stack is restored to a depth of 2 when THROW is
\ executed. The 77 ensures the top of stack value is known for
\ the results check
